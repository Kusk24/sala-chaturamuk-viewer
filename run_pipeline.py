...
Multiple captures, kept separate:

    python run_pipeline.py --version-name salathai_version2

Reads from data/raw_versions/salathai_version2/ instead of data/raw/, and
writes every output under that name (data/selected_salathai_version2/,
output/sequence_salathai_version2/, viewer/manifest_salathai_version2.json)
so it never overwrites another version's result. Registers itself in
viewer/versions.json, which the viewer reads to build the version-switcher
buttons. Drop each new capture in its own data/raw_versions/<name>/ folder
first.
...
def register_version(name, manifest_path):
    ...
    versions_json = os.path.join(ROOT, "viewer", "versions.json")
    versions = {"versions": []}
    if os.path.isfile(versions_json):
        with open(versions_json) as fh:
            versions = json.load(fh)

    entry = {
        "name": name,
        "label": name.replace("_", " ").replace("salathai", "Sala Thai").strip().title(),
        # .js, not .json: the viewer loads this as a <script> so it also works
        # over file://, same reason manifest.py writes a .js twin of the JSON.
        "manifest": os.path.splitext(os.path.basename(manifest_path))[0] + ".js",
    ...
    with open(versions_json, "w") as fh:
        json.dump(versions, fh, indent=2)
    with open(os.path.join(ROOT, "viewer", "versions.js"), "w") as fh:
        fh.write("// Generated by run_pipeline.py -- do not edit.\n")
        fh.write("window.SALA_VERSIONS = ")
        json.dump(versions, fh, indent=2)
        fh.write(";\n")
...
def main():
    ...
    raw = os.path.join(ROOT, "data", "raw_versions", args.version_name) if args.version_name else os.path.join(ROOT, "data", "raw")
    frames = os.path.join(ROOT, "data", f"frames{suffix}")
    selected = os.path.join(ROOT, "data", f"selected{suffix}")
    sequence = os.path.join(ROOT, "output", f"sequence{suffix}")
    metrics = os.path.join(ROOT, "output", f"metrics{suffix}")
    matches = os.path.join(ROOT, "output", f"matches{suffix}.json")
    manifest = os.path.join(ROOT, "viewer", f"manifest{suffix}.json")
    ...
    if args.input:
        ...
        if args.version_name and not os.path.isdir(raw):
            sys.exit(f"No such folder: {rel(raw)}/\nCreate it and put this version's capture inside.")
    ...
    run_record_path = os.path.join(ROOT, "output", f"pipeline_run{suffix}.json")
